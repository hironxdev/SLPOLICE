  ...
  return (
    <div className="p-8 lg:p-10 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Incident Response</h2>
          <p className="text-sm text-slate-500 font-medium mt-1">Manage active security threats and execute responder playbooks.</p>
        </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <Activity className="w-4 h-4 text-blue-600" /> Active Threat Feed
              </h3>
            ...
            <div className="divide-y divide-slate-100">
              {loading ? (
                ...
                <div key={incident.id} className="p-8 hover:bg-slate-50/50 transition-all group">
                  <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-8">
                    <div className="space-y-4 flex-1">
                      <div className="flex items-center gap-3">
                        <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold border ${
                          incident.severity === 'CRITICAL' ? 'bg-rose-50 text-rose-700 border-rose-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                        }`}>
                          {incident.severity}
                        </span>
                        <span className="text-[11px] font-mono text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{incident.id}</span>
                        <div className="flex items-center gap-2 px-2 py-0.5 bg-blue-50 rounded-full border border-blue-100">
                          <div className={`w-1.5 h-1.5 rounded-full ${incident.status === 'CONTAINING' ? 'bg-rose-500 animate-pulse' : 'bg-blue-600'}`}></div>
                          <span className="text-[10px] font-bold text-blue-700 uppercase tracking-wide">{incident.status}</span>
                        </div>
                      </div>
                      <h4 className="text-lg font-bold text-slate-900">{incident.name}</h4>
                      <div className="flex items-center gap-6 text-xs font-medium text-slate-500">
                        <div className="flex items-center gap-2"><Users size={14} className="text-slate-400" /> {incident.assigned}</div>
                        <div className="flex items-center gap-2"><Clock size={14} className="text-slate-400" /> {incident.time} elapsed</div>
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                ...
                <button key={pb.name} className="w-full flex items-center justify-between p-4 rounded-lg bg-slate-50 border border-slate-200 hover:border-blue-300 hover:bg-blue-50/30 transition-all group">
                  <span className="text-xs font-semibold text-slate-600 group-hover:text-blue-700">{pb.name}</span>
                  <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-1.5 py-0.5 rounded">{pb.level}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-700 to-blue-800 rounded-xl p-8 space-y-4 shadow-xl shadow-blue-200">
            <div className="flex items-center gap-4">
              ...
              <h3 className="text-sm font-bold text-white tracking-wide">Strike Ready</h3>
            </div>
            <p className="text-xs font-medium text-blue-100 leading-relaxed">
              Tactical Response Teams are standing by for national security mitigation orders.
            </p>
