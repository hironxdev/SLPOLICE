        ...
        setActiveTargets(
          data
            .map((v) => ({
              ...
              alias:
                v.forensics?.isp === "Dialog Telekom Plc"
                  ? "MOBILE_UPLINK"
                  : "REMOTE_VECTOR",
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-50 border border-blue-100 rounded-lg flex items-center justify-center shadow-sm">
              <Radar
                className={`w-6 h-6 text-blue-700 ${pulse ? "animate-pulse scale-105" : "transition-all"}`}
              />
            </div>
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">
              Geospatial Intelligence{" "}
              <span className="text-blue-600 text-sm font-medium ml-2 px-2 py-0.5 bg-blue-50 rounded border border-blue-100">
                v1.4
              </span>
            </h2>
          </div>
          <p className="text-sm text-slate-500 font-medium">
            Multi-Vector Location Tracing & Triangulation System
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-4 gap-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <Database className="w-4 h-4 text-blue-600" /> Target
              ...
              <div className="flex items-center gap-4">
                <div className="relative group">
                  ...
                  <input
                    type="text"
                    placeholder="Filter by ID or IP..."
                    className="bg-white border border-slate-200 rounded-lg py-2 pl-11 pr-4 text-xs font-semibold text-slate-700 outline-none focus:border-blue-300 transition-all placeholder:text-slate-400 shadow-sm"
                  />
                </div>
              </div>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    <tr>
                      <td
                        colSpan={4}
                        className="px-8 py-20 text-center text-slate-400 text-sm font-medium animate-pulse"
                      >
                        Initializing orbital tracking downlink...
                      </td>
                    </tr>
                  ) : (
                    activeTargets.map((t) => (
                      ...
                      >
                        ...
                        <td className="px-8 py-6">
                          <div className="space-y-1.5 max-w-[120px]">
                            ...
                            <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full ${t.fusion_score > 90 ? "bg-emerald-500" : "bg-blue-600"} transition-all`}
                                style={{ width: `${t.fusion_score}%` }}
                              ></div>
                            </div>
                          </div>
                        </td>
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-2.5">
                            <div
                              className={`w-1.5 h-1.5 rounded-full ${t.status === "LOCKED" ? "bg-rose-500 animate-pulse" : t.status === "TRACKING" ? "bg-blue-600" : "bg-slate-300"}`}
                            ></div>
                            <span
                              className={`text-[10px] font-bold uppercase tracking-wider ${t.status === "LOCKED" ? "text-rose-600" : "text-slate-500"}`}
                            >
                              {t.status}
                            </span>
                          </div>
        ...
        <div className="space-y-8">
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6 shadow-sm overflow-hidden relative">
            <div className="absolute -top-10 -right-10 opacity-5 text-blue-700 pointer-events-none">
              <Globe size={160} />
            </div>
            <div className="space-y-5 relative z-10">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <Activity className="w-4 h-4 text-blue-600" /> Operational
                Signals
              </h3>
              <div className="space-y-4">
                {[
                  {
                    label: "Triangulation Sync",
                    val: "ACTIVE",
                  ...
                  <div
                    key={s.label}
                    className="flex justify-between items-center bg-slate-50/50 p-2.5 rounded-lg border border-slate-100"
                  >
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">
                      {s.label}
                    </span>
                    <span
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${s.color}`}
                    >
                      {s.val}
                    </span>
                  </div>
          ...
          <div className="bg-gradient-to-br from-rose-600 to-rose-700 p-8 rounded-xl space-y-4 shadow-xl shadow-rose-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[9px] font-semibold text-rose-100 opacity-80 uppercase">
                  Code Red Isolation
                </p>
              </div>
            </div>
            <p className="text-xs font-medium text-rose-50 leading-relaxed">
              Unauthorized cross-border access detected on secondary vectors.
              Initiating hardware isolation.
            </p>
